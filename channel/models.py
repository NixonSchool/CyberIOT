...
class Channel(models.Model):
    ...
    subscribers = models.ManyToManyField(
        settings.AUTH_USER_MODEL, related_name="video_subscribers", blank=True
    )
    user = models.OneToOneField(settings.AUTH_USER_MODEL, on_delete=models.CASCADE)
    description = RichTextField(verbose_name="Description", blank=True)
    avatar = models.ImageField(
        upload_to='channel/avatar/', default="images/channel/avatar/avatar.png"
    )
    banner_image = models.ImageField(
        upload_to='channel/banner_image/', default="images/channel/avatar/avatar.png"
    )
    ...
    def create_for_user(user):
        """Create a Channel instance for a given user."""
